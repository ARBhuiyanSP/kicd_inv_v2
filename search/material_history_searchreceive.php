				...
				<table id="" class="table table-bordered">
					...
					<tbody>
						<?php
							...
							$totalout = 0;
							$sqlall	=	"SELECT * FROM `inv_materialbalance` WHERE `mb_materialid` = '$material_name' AND mbtype='Receive' AND `mb_date` BETWEEN '$from_date' AND '$to_date';";
							...
							{	
								...
								$outQty = $rowall['mbin_qty'];
								$totalout += $rowall['mbin_qty'];
						...
						<tr style="text-align:right;font-weight:bold;">
							...
							<td>
								<?php 
									$sqlout = "SELECT sum(mbin_qty) FROM `inv_materialbalance` WHERE `mb_materialid` = '$material_name' AND `mb_date` BETWEEN '$from_date' AND '$to_date'";
									$resultout = mysqli_query($conn, $sqlout);
									for($i=0; $rowout = mysqli_fetch_array($resultout); $i++){
									$totalOut=number_format((float)$rowout['sum(mbin_qty)'], 2, '.', '');
