		...
		<div class="col-md-10" id="printableArea">
			<div class="row">
				<div class="col-sm-12">	
					<center>
						<p>
							...
							<span>Material Issued Report</span></br>
				...
				<table id="" class="table table-bordered">
					...
					<tbody>
						...
						<?php
							...
							$totalAmount = 0;
							//$mrr_no = $row['mrr_no'];
							$sqlall	=	"SELECT * FROM `inv_materialbalance` WHERE `mb_materialid` LIKE '%$cat_code%' AND `mbtype`='Issue' AND `mb_date` BETWEEN '$from_date' AND '$to_date';";
						...
						<tr>
									...
									<td style="text-align:center">
										<?php 
										$sql2 = "SELECT sum(`mbout_qty`) FROM `inv_materialbalance` WHERE `mb_materialid` LIKE '%$cat_code%' AND `mbtype`='Issue' AND `mb_date` BETWEEN '$from_date' AND '$to_date'";
										$result2 		= mysqli_query($conn, $sql2);
										for($i=0; $row2 = mysqli_fetch_array($result2); $i++){
										$totalIssued	=$row2['sum(`mbout_qty`)'];
										echo $totalIssued ;
										}
										?>
									</td>
									...
									<?php 
										$sql3 = "SELECT sum(`mbout_val`) FROM `inv_materialbalance` WHERE `mb_materialid` LIKE '%$cat_code%' AND `mbtype`='Issue' AND `mb_date` BETWEEN '$from_date' AND '$to_date'";
										$result3		= mysqli_query($conn, $sql3);
										for($i=0; $row3 = mysqli_fetch_array($result3); $i++){
										$totalAmount	= number_format((float)$row3['sum(`mbout_val`)'], 2, '.', '');
										echo $totalAmount ;
										}
										?>
									</td>
				...
				<center><div class="row">
					<div class="col-sm-6"></br></br>--------------------</br>Issuer Signature</div>
