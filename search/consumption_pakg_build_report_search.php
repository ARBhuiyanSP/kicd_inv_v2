                        ...
                        <tr>  
							<td>
                                ...
                                <select class="form-control select2" id="building_id" name="building_id" required>
                                    ...
                                    if (isset($projectsData) && !empty($projectsData)) {
                                        foreach ($projectsData as $data) {
													if($_GET['building_id'] == $data['building_id']){
													$selected	= 'selected';
													}else{
													$selected	= '';
													}
                                            ?>
                                            <option value="<?php echo $data['building_id']; ?>" <?php echo $selected; ?>><?php echo $data['building_id']; ?></option>
                                            <?php
                                        }
                                    }
                                    ?>
				...
				<div class="col-sm-12">	
					<center>
						<p>
							...
							<b>Package and Bulding Wise Consumption Report</b><br>
							...
							?>
							<b>Package : <?php echo (isset($dataresult) && !empty($dataresult) ? $dataresult->name : ''); ?></b><br> <b>Building : <?php echo $building_id; ?></b><br>
						</p>
					</center>
				</div>
			</div>
				<table id="" class="table table-bordered table-striped ">
					...
					<?php
						if($_SESSION['logged']['user_type'] !== 'whm'){
							$sql	=	"SELECT * FROM `qry_inv_issue` WHERE `project_id` = '$project_id' AND `package_id` = '$package_id' AND `building_id` = '$building_id' AND `issue_date` BETWEEN '$from_date' AND '$to_date'  GROUP BY `material_id`";
						}else{
							$sql	=	"SELECT * FROM `qry_inv_issue` WHERE `warehouse_id` = '$warehouse_id' AND `project_id` = '$project_id' AND `package_id` = '$package_id' AND `building_id` = '$building_id' AND `issue_date` BETWEEN '$from_date' AND '$to_date'  GROUP BY `material_id`";
						}
					...
					?>
						<tr>
							...
							<td><?php
							if($_SESSION['logged']['user_type'] !== 'whm'){
								$sqloutqty = "SELECT SUM(`issue_qty`) AS totalout FROM `qry_inv_issue` where `project_id` = '$project_id' AND `package_id` = '$package_id' AND `building_id` = '$building_id' AND `issue_date` BETWEEN '$from_date' AND '$to_date' and  `material_id` = '$material_id' GROUP BY `material_id`";
							}else{
								$sqloutqty = "SELECT SUM(`issue_qty`) AS totalout FROM `qry_inv_issue` where `warehouse_id` = '$warehouse_id' AND `project_id` = '$project_id' AND `package_id` = '$package_id' AND `building_id` = '$building_id' AND `issue_date` BETWEEN '$from_date' AND '$to_date' and  `material_id` = '$material_id' GROUP BY `material_id`";
							}
